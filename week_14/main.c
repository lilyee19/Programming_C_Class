...
int getAlivePlayer(void)
{
   ...
   for (i=0; i<N_PLAYER;i++)
   {
   	if (player_Status[i] == PLATERSTAUTS_LIVE)
   	{
   		flag_end = 0;
   		break;
	}
...
// ----- EX. 4 : player ------------
        dieResult = rolldie();
        play_position[turn]+= dieResult;
    
        if (play_position[turn] > N_BOARD) }
            printf("Player %s reached the end of the board!\n", player_name[i]);
            player_status[turn] = PLAYERSTATUS_END; 
        } else {
            printf("Player %s moved to position %d\n", player_name[i], play_position[turn]);
        }
